<template>
	...
	<view 
		...
		:style="{
				...
				height: 'calc(200% - ' + pageTop + 'px)',
				maxHeight: 'calc(200% - ' + pageTop + 'px)'
			}"
	...
	<view 
		...
		:style="{
				transform: 'translateY('+ pageDeviation +'px)',
				transition: pageTransition + 's',
				height: 'calc(200vh - ' + pageTop + 'px)',
				maxHeight: 'calc(200vh - ' + pageTop + 'px)'
			}"
		@touchstart="pageTouchstart"
		@touchmove="pageTouchmove"
		@touchend="pageTouchend"
	...
	export default {
		...
		methods: {
			...
			startPulldownRefresh(){
				...
				this.pageDeviation = uni.upx2px(90+50);
				this.$emit('refresh');
			},
			//结束下拉刷新
			endPulldownRefresh(){
...
<style>
	...
	@keyframes loading {
		...
		100% {
			transform: translateY(4upx)  scaleX(1.4);
		}
	}
