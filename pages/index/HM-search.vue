<template>
	<view class="content">
		...
		<view class="search-keyword" @touchstart="blur">
			...
			<view class="nav-type">
				<view :key="indext" class="nav-type-item-div" v-for="(itemt,indext) in navtype" @click="changeNav(indext)">
					<text  class="nav-type-item" :class="currtype==indext?'active':''" :key="indext">{{itemt.val}}</text>
				</view>
			</view>
			<scroll-view class="keyword-box" v-show="!isShowKeywordList" scroll-y>
				...
				<view class="keyword-block">
					...
					<view class="keyword" v-if="forbid==''">
						<view v-for="(keyword,index) in hotKeywordList" @tap="doSearch(keyword.keywords)" :key="index">{{keyword.keywords}}</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			loadDefaultKeyword() {
				...
				this.defaultKeyword = this.hotKeywordList[0].keywords||"请输入关键字";
			},
			...
			loadHotKeyword() {
				//定义热门搜索关键字
				this.$api.HotSearch(10).then(res => {
					let {
						status,
						content
					} = res.data;
					if (status == 200) {
						this.hotKeywordList = content;
					}
			...
			navigateToShop(value) {
				...
				if(this.currtype==2){
					uni.navigateTo({
						url: `/pages/index/search-pshop?keywords=${value}`,
						success: res => {},
						fail: () => {},
						complete: () => {}
					});
					...
					uni.navigateTo({
						url: `/pages/index/search-jshop?keywords=${value}`,
						success: res => {},
						fail: () => {},
						complete: () => {}
					});
					...
					uni.navigateTo({
						url: `/pages/index/search-shop?keywords=${value}`,
						success: res => {},
						fail: () => {},
						complete: () => {}
					});
...
<style lang="scss">
	...
	.nav-type{
		...
		.active{	
			font-weight: bold;
			color: #ED7236;
			border-bottom: 8upx solid #ED7236;	
		}
		.nav-type-item-div{	
			width: 150upx;
			height: 80upx;
