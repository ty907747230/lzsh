...
<script>
	...
	import Nav from "@/components/nav-j.vue";
	import ShopListOne from "@/components/shop-news-j.vue"; //单列
	import ShopListTwo from "@/components/shop-two-j.vue"; //双列
	...
	export default {
		...
		methods: {
			...
			getType(data) {
				...
				this.$api.jd_goods_list(this.pageno, this.pagesize,this.keywords, data.type, data.moren,this.youquan).then(res => {
					let {
						data
					} = res.data;
					this.pageno++;
